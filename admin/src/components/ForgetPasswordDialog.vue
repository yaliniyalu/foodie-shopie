...
async function verifyOtp() {
  try {
    error.value = null
    loading.value = true
    const res = await api.get("auth/recover-password/verify", {params: {email: email.value, otp: code.value}})
    step.value = 3;
  } catch (e) {
    error.value = e.response.data.message
  } finally {
...
async function changePassword() {
  ...
  try {
    loading.value = true
    await api.post("auth/recover-password/change", {password: password.value, email: email.value, otp: code.value})
