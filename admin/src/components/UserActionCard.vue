<template>
  <div>
    <q-card v-bind="props2" ref="c1" bordered flat @click="isShown = !isShown">
      <q-card-section class="row" v-if="isShown && props.user">
      ...
      <q-card-section class="row" v-else>
        <strong>{{ action }}</strong>
        <strong v-if="props.user">&nbsp; by &nbsp;{{ props.user['name'] }}</strong>
        <strong>&nbsp; at &nbsp;{{ humanDateTime(date) }}</strong>
      </q-card-section>

      <q-separator/>

      <q-card-section class="text-center" v-if="isShown && props.user">
        <div class="row no-wrap">
          ...
          <div class="column q-ml-md text-left">
            ...
            <span class="text-grey-8">{{props.user['phone'] }}</span>
          </div>
        </div>
      </q-card-section>
    </q-card>
...
<script setup>
import {getAvatarUrl, humanDateTime} from "src//js/utils";
