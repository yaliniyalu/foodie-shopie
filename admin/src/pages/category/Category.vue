<template>
  <q-page class="q-gutter-md q-pa-md">
    <q-card>
      <q-card-section>
        <div class="row items-center no-wrap">
          <div class="col">
            <div class="text-h6 q-ma-none">Category</div>
          </div>

          <div class="col-auto">
            <q-btn color="primary" flat rounded icon="add" label="Add New" to="/category/new"/>
          </div>
        </div>
      </q-card-section>
        ...
        >
          ...
          <template v-slot:top>
            <div class="q-col-gutter-md row">
              ...
              />
              <CategorySelect class="col" outlined dense stack-label label="Parent" clearable options-dense use-input emit-value v-model="filter._parentId.value" :readonly="filter._parentId.readonly"/>
            </div>
    ...
    <NavigationDrawer :path="['/category/:id', '/category/:id/edit', '/category/new']" @update="trigger"/>
  </q-page>
</template>
...
import {Columns} from "src/js/model-helpers/category";
...
withResponseParser((data) => [data.data.category, data.data.totalCount])

withOptions({
  url: 'category',
  storageKey: 'category-list'
})

function onRowClick(evt, row, index) {
  router.currentRoute.value.path === '/category'
    ? router.push('/category/' + row.id)
    : router.replace('/category/' + row.id)
}
</script>
