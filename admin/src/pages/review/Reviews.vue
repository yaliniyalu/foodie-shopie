<template>
  <q-page class="q-gutter-md q-pa-md">
    <q-card>
      <q-card-section>
        <div class="row items-center no-wrap">
          <div class="col">
            <div class="text-h6 q-ma-none">Reviews</div>
          </div>
        </div>
      </q-card-section>
      ...
      <q-card-section>
        ...
        >
          <template v-slot:body-cell-customer="props">
            <q-td key="customer" :props="props" v-if="props.value">
              {{ props.value.phone }}
              <q-tooltip>
          ...
          <template v-slot:top>
            <div class="q-col-gutter-md row">
              ...
              <SelectItem class="col mw-150" outlined dense stack-label label="Item" clearable v-model="filter._itemId.value" :readonly="filter._itemId.readonly" map-options emit-value/>
            </div>
    ...
    <NavigationDrawer path="/review/:id" @update="trigger"/>
  </q-page>
</template>
...
import {Columns, Status} from "src/js/model-helpers/reviews";
import RatingStar from "components/badge/RatingStarBadge";
...
withResponseParser((data) => [data.data.reviews, data.data.totalCount])

withOptions({
  url: 'review',
  storageKey: 'review-list'
})

function onRowClick(evt, row, index) {
  router.currentRoute.value.path === '/review'
    ? router.push('/review/' + row.id)
    : router.replace('/review/' + row.id)
}
</script>
